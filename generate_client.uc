...
if (!isEmpty(main_node)) {
	...
	uci.foreach(uciconfig, uciroutingnode, (cfg) => {
		...
		} else if (cfg.node === 'all_nodes') {
			/* Build a selector outbound from user-chosen nodes */
			const selector_nodes = cfg.selector_nodes || [];
			push(config.outbounds, {
				type: 'selector',
				tag: 'cfg-' + cfg['.name'] + '-out',
				outbounds: map(selector_nodes, (k) => `cfg-${k}-out`)
			});
			/* Defer node outbound generation to the post-loop block, same as urltest */
			urltest_nodes = [...urltest_nodes, ...filter(selector_nodes, (l) => !~index(urltest_nodes, l))];
		} else {
