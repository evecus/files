...
return view.extend({
	...
	render(data) {
		...
		so.value('all_nodes', _('All nodes (Selector)'));
		for (let i in proxy_nodes)
		...
		so.depends({'node': 'all_nodes', '!reverse': true});  /* domain_resolver */
		so.modalonly = true;
		...
		for (let i in hp.dns_strategy)
			so.value(i, hp.dns_strategy[i]);
		so.depends({'node': 'urltest', '!reverse': true});
		so.depends({'node': 'all_nodes', '!reverse': true});  /* domain_strategy */
		so.modalonly = true;
		...
		so.multiple = false;
		so.noaliases = true;
		so.depends({'outbound': '', 'node': /^((?!urltest$|all_nodes$).)+$/});
		so.modalonly = true;
		...
		so.depends({'node': 'urltest', '!reverse': true});
		so.depends({'node': 'all_nodes', '!reverse': true});  /* outbound */
		so.editable = true;
		...
		so = ss.option(hp.CBIStaticList, 'selector_nodes', _('Selector nodes'),
			_('List of nodes in this selector group.'));
		for (let i in proxy_nodes)
			so.value(i, proxy_nodes[i]);
		so.depends('node', 'all_nodes');
		so.validate = function(section_id) {
